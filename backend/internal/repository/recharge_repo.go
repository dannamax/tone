...
func (r *RechargeRepo) Create(ctx context.Context, o *model.RechargeOrder) (*model.RechargeOrder, error) {
	...
	query := `INSERT INTO recharge_orders (id, user_id, package_id, channel, amount, currency, quota_granted, status, gateway_order_id, receipt_data, created_at) 
			  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	_, err := r.db.ExecContext(ctx, query,
		o.ID, o.UserID, o.PackageID, o.Channel, o.Amount, o.Currency, o.QuotaGranted, o.Status, o.GatewayOrderID, o.ReceiptData, o.CreatedAt)
	if err != nil {
		return nil, err
	}
	return o, nil
...
func (r *RechargeRepo) FindByID(ctx context.Context, id string) (*model.RechargeOrder, error) {
	...
	query := `SELECT id, user_id, package_id, channel, amount, currency, quota_granted, status, gateway_order_id, receipt_data, paid_at, created_at 
			  FROM recharge_orders WHERE id = ?`
	var paidAt sql.NullTime
	err := r.db.QueryRowContext(ctx, query, id).Scan(
		&o.ID, &o.UserID, &o.PackageID, &o.Channel, &o.Amount, &o.Currency, &o.QuotaGranted, &o.Status, &o.GatewayOrderID, &o.ReceiptData, &paidAt, &o.CreatedAt)
...
// MarkPaid 幂等标记订单为已支付并发放额度。
// 仅当订单处于 created 时才切换并加额度；已 paid 则直接返回成功（重复通知安全）。
func (r *RechargeRepo) MarkPaid(ctx context.Context, id, gatewayOrderID string, quotaGranted int, addQuota func(ctx context.Context, userID string, n int) error) (*model.RechargeOrder, error) {
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE recharge_orders SET status = ?, gateway_order_id = ?, quota_granted = ?, paid_at = ?, updated_at = ? 
		 WHERE id = ? AND status = ?`,
		model.OrderStatusPaid, gatewayOrderID, quotaGranted, time.Now(), time.Now(), id, model.OrderStatusCreated)
	if err != nil {
		return nil, err
	}
	...
	// 发放额度
	if err := addQuota(ctx, o.UserID, quotaGranted); err != nil {
		return nil, err
	}
	o.Status = model.OrderStatusPaid
	o.GatewayOrderID = gatewayOrderID
	o.QuotaGranted = quotaGranted
	t := time.Now()
	o.PaidAt = &t
	return o, nil
}
